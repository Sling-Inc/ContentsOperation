...
import {
  performGoogleDocumentOCR,
  visualizeOcrResults,
} from "#root/utils/cloudVision.js";
...
async function processImageDirectory(imageDir, outputDir, isDebug) {
  const imageFiles = (await fs.readdir(imageDir)).filter((f) =>
  ...
  const worker = async (workerId) => {
    while (taskQueue.length > 0) {
      ...
      Logger.info(
        `[Worker ${workerId}] Processing ${processedCount}/${imageFiles.length}: ${imageFile}`
      );
      ...
      try {
        ...
        if (ocrResult) {
          ...
          if (isDebug) {
            Logger.debug(
              `[Worker ${workerId}] Visualizing OCR for ${imageFile}...`
            );
            const visualizedBuffer = await visualizeOcrResults(
              imageBuffer,
              ocrResult
            );
            if (visualizedBuffer) {
        ...
        Logger.error(
          `[Worker ${workerId}] Failed to process ${imageFile}: ${workerError.message}`
        );
      }
    }
  };
...
async function main() {
  ...
  if (args.length < 2) {
    console.error(
      "Usage: node scripts/B01_cloudVisionOCR <inputBaseDir> <outputBaseDir> [--debug] [--examType <type>]"
    );
    process.exit(1);
  }
  ...
  Logger.section(
    `Starting OCR processing for base directory: ${absoluteInputBaseDir}`
  );
  ...
  try {
    const subDirs = await fs.readdir(absoluteInputBaseDir, {
      withFileTypes: true,
    });
