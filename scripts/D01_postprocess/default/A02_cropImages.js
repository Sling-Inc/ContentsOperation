...
async function main() {
  Logger.section("D01-default-02 Crop Images Start");
  try {
    ...
    for (const bboxFile of bboxFiles) {
      ...
      for (const id in itemsById) {
        ...
        for (const item of items) {
          ...
          try {
            ...
            const cropBuffer = await sharp(imagePath)
              .extract({ left: Math.round(x1), top: Math.round(y1), width: Math.round(x2 - x1), height: Math.round(y2 - y1) + 2 })
              .png()
              .toBuffer();
        ...
        if (cropBuffers.length > 0) {
          const metadatas = await Promise.all(cropBuffers.map(b => sharp(b).metadata()));
          ...
          for (let i = 0; i < cropBuffers.length; i++) {
            compositeOperations.push({ input: cropBuffers[i], top: currentHeight, left: 0 });
            currentHeight += metadatas[i].height;
          }

          const finalImageBuffer = await sharp({ create: { width: maxWidth, height: totalHeight, channels: 4, background: { r: 255, g: 255, b: 255, alpha: 1 } } })
    ...
    Logger.endSection("D01-default-02 Crop Images Finished");
    Logger.close();
  }
}
