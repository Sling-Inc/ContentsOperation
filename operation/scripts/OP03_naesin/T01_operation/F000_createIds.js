...
export async function F000_createIds() {
  ...
  Logger.section(`내신 ID 생성 작업을 시작합니다... ${TARGET_DIR}`);
  ...
  for (const subdir of subdirectories) {
    ...
    Logger.info(`✅ ${subdirName} ID 생성 완료`);
  }

  Logger.info(`총 Material ID: ${materialIdSet.size}`);
  Logger.info(`총 ExamPaper ID: ${examPaperIdSet.size}`);
  Logger.info(`총 Problem ID: ${problemIdSet.size}`);
  Logger.info(`총 Passage ID: ${passageIdSet.size}`);

  Logger.endSection("ID 생성 작업 완료");
}