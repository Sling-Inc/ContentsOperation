...
export class ScriptLogger {
   ...
   */
  constructor(logPath = path.join(process.cwd(), "_logs")) {
    const mainScriptPath = process.argv[1];
    const logName = mainScriptPath ? path.parse(mainScriptPath).name : undefined;

    if (logPath) this.setLogFilePath(logPath, logName);
    this.__writeQueue = [];
  ...
  __processQueue() {
    ...
    const { message } = this.__writeQueue[0];
